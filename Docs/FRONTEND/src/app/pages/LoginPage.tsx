...
export function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    if (isSignUp && !userType) {
      toast.error('Selecciona si eres consultor o empresa antes de continuar con Google.');
      return;
    }

    if (isSignUp && avatarFile) {
      await storePendingAvatar(avatarFile);
    }
    ...
    if (isSignUp && userType) {
      ...
      if (fullName.trim()) {
        callbackUrl.searchParams.set('fullName', fullName.trim());
      }

      if (city.trim()) {
        callbackUrl.searchParams.set('city', normalizeCity(city));
      }
    }
